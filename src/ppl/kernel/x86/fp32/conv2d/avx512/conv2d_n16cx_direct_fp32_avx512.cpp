...
#include "ppl/kernel/x86/fp32/conv2d/avx512/conv2d_n16cx_direct_kernel_fp32_avx512.h"
...
int64_t conv2d_n16cx_direct_fp32_avx512_executor::cal_ic_l2_blk(const conv2d_param &param)
{
...
void conv2d_n16cx_direct_fp32_avx512_executor::init_preproc_param()
{
...
void conv2d_n16cx_direct_fp32_avx512_executor::cal_kernel_tunning_param()
{
...
uint64_t conv2d_n16cx_direct_fp32_avx512_executor::cal_temp_buffer_size()
{
...
ppl::common::RetCode conv2d_n16cx_direct_fp32_avx512_executor::execute()
{
    ...
    const int64_t src_dh_stride  = cp.dilation_h * src_w * CH_DT_BLK();
    const int64_t src_dw_stride  = cp.dilation_w * CH_DT_BLK();
    const int64_t dst_b_stride   = round_up(dst_shape_->GetDim(1), CH_DT_BLK()) * dst_h * dst_w;
    const int64_t dst_g_stride   = sp.padded_oc * dst_h * dst_w;
    const int64_t dst_ocb_stride = dst_h * dst_w * CH_DT_BLK();
    const int64_t dst_h_stride   = dst_w * CH_DT_BLK();
...
ppl::common::RetCode conv2d_n16cx_direct_fp32_avx512_manager::gen_cvt_weights(const float *filter, const float *bias)
{
    ...
    const int64_t ic_l2_blk = conv2d_n16cx_direct_fp32_avx512_executor::cal_ic_l2_blk(param_);
...
bool conv2d_n16cx_direct_fp32_avx512_manager::is_supported()
{
...
conv2d_fp32_executor *conv2d_n16cx_direct_fp32_avx512_manager::gen_executor()
{
    return new conv2d_n16cx_direct_fp32_avx512_executor(&param_, cvt_filter_, cvt_bias_);
}
